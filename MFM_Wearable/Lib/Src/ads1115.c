...
{
	// 기본 설정은 기존 Init 함수와 동일하게 수행
	ADS1115_Init(handler, setDataRate, setPGA);

	// 연속 변환 모드로 설정값을 구성 (MODE 비트 = 0)
	ADS1115_config[0] = ADS1115_OS | muxPort | setPGA | 0; // MODE 비트를 0으로 설정
...
HAL_StatusTypeDef ADS1115_readContinuous(float *voltage)
{
	...
	{
		// 미리 계산된 계수를 사용하여 전압으로 변환
		*voltage = (float) (((int16_t) (ADS1115_rawValue[0] << 8) | ADS1115_rawValue[1]) * ADS1115_voltCoef / 1000.0f); // mV를 V로 바꾸기 위해 1000으로 나눔
		return HAL_OK;
	}
